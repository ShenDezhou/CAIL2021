...
class NERNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.sentence_encoder = nn.LSTM(64, 16, num_layers=1, batch_first=True,
        #                                 bidirectional=True)
        #20 topic, 24 features
        self.fc = nn.Linear(20, 1000)
        # self.dropout = nn.Dropout(0.01)
        self.fc1 = nn.Linear(1000, 64)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(64, 1000)
        self.fc3 = nn.Linear(1000, 24)
        self.sigmoid = nn.LogSigmoid()
        # self.crf = CRF(config.num_classes, batch_first=True)
        # self.criterion = nn.MultiLabelSoftMarginLoss(reduction='sum') #label
    ...
    def forward(self, topic, properties=None):
        y = self.fc(topic)
        origin = y
        y = self.fc1(y)
        y = self.sigmoid(y)
...
class BERTXLNet(nn.Module):
    """
        NERNet : Lstm+CRF
    """

    def __init__(self, config):
        super(BERTXLNet, self).__init__()
        ...
        # self.sentence_encoder = nn.LSTM(config.hidden_size, config.sent_hidden_size, num_layers=1, batch_first=True,
        #                                 bidirectional=True)
        self.emission = nn.Linear(config.hidden_size, config.num_classes)
        self.crf = CRF(config.num_classes, batch_first=True)
        self.label_type = nn.Linear(config.hidden_size, config.num_classes)
        self.criterion = nn.MultiLabelSoftMarginLoss(reduction='mean') #label

    def forward(self, topic, properties):

        type_loss = self.criterion(topic, properties)
        return type_loss
