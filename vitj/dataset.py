...
class TsinghuaDog(Dataset):
    def __init__(self, root_dir, batch_size, part='train', train=False, shuffle=False, transform=None, num_workers=1, sample_rate=0):
        ...
        if part == 'train':
            ...
            self.root_dir = os.path.join(self.root_dir, f'cifar{self.num_classes}_train/')
        elif part == 'val':
            list_path = os.path.join(self.root_dir, 'TrainAndValList/validation.lst')
            self.root_dir = os.path.join(self.root_dir, f'cifar{self.num_classes}_test/')
...
class TsinghuaDogExam(Dataset):
    def __init__(self, root_dir, batch_size, name_list=None, train=False, shuffle=False, transform=None, num_workers=1):
        super().__init__()
        self.root_dir = root_dir
        self.batch_size = batch_size
        self.train = train
